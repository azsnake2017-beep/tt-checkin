...
function showUserInfoModal(uid) {
  ...
  if(mInv) mInv.style.display = 'none'; // Глушим старый, проблемный контейнер
  
  var modal = document.getElementById('user-info-modal');
  if (modal) modal.style.display = 'flex';
  
  // КРИТИЧЕСКИЙ ФИКС: Заставляем Firebase тянуть свежие данные с сервера, чтобы ракетки всегда были актуальны
  var userPromise = db.collection('users').doc(uid).get({ source: 'server' }).catch(function() {
      return db.collection('users').doc(uid).get(); // Фолбэк на кэш, если пропал интернет
  });
  ...
  Promise.all([
    ...
    // Вывод Админской плашки и уникальных кастомных плашек
    ...
    // Вставляем имя + ВСЕ плашки в шапку окна
    ...
    // --- ГАРАНТИРОВАННАЯ ОТРИСОВКА РАКЕТКИ ---
    // Формируем HTML инвентаря
    var invHtml = '';
    if (u.blade || u.rubberL || u.rubberR) {
      invHtml += '<div class="inventory-box" style="margin-top: 8px;">';
      ...
      if (typeof getInventoryRowHtml === 'function') {
        ...
        // Резервный вариант, если стилей нет под рукой
    ...
    // Отрисовываем всё тело карточки и приклеиваем инвентарь (invHtml) прямо сюда!
    document.getElementById('info-modal-content-area').innerHTML = uidHtml +
      ...
      invHtml; // <--- Ракетка гарантированно появляется в карточке!
