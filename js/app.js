...
function showUserInfoModal(uid) {
  ...
  var medalsEl = document.getElementById('info-modal-medals');
  var invEl = document.getElementById('info-modal-inventory');
  ...
  Promise.all([
    ...
    if (medalsEl) {
      ...
      if (m.gold > 0 || m.silver > 0 || m.bronze > 0 || u.tournamentsPlayed > 0) {
        medalsEl.innerHTML = '<div class="medal-item">🏆 ' + (u.tournamentsPlayed || 0) + '</div><div class="medal-item">🥇 ' + (m.gold || 0) + '</div><div class="medal-item">🥈 ' + (m.silver || 0) + '</div><div class="medal-item">🥉 ' + (m.bronze || 0) + '</div>'; 
    ...
    if (historyEl) historyEl.innerHTML = '<span class="empty-note">Ошибка загрузки профиля</span>';
  });
}
